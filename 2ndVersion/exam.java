...
public class exam {
  ...
  /*static public String jiecheng(String[] X) {
        double result = Double.parseDouble(X[X.length].substring(1, X.length - 1));
        double s=result;
        for (; result > 0; result--) {
            s*= result - 1;
        }
    ...
    static public String Dooperation(String X) {
       // String XXXXX= jiecheng(divide(X));
        String XXXX = ostwo(divide(X));
        String s1 = sin(divide(XXXX));
